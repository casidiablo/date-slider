...
public class ScrollLayout extends LinearLayout {
    ...
    private DateSlider.Labeler mLabeler;
    ...
    void reScrollTo(int x, int y, boolean notify) {
        if (getChildCount() > 0) {
            ...
            if (mFirstElemOffset - mInitialOffset > mObjWidth / 2) {
                ...
                mFirstElemOffset = ((mFirstElemOffset - mInitialOffset - mObjWidth / 2) % mObjWidth) + mInitialOffset - mObjWidth / 2;
                ...
                mFirstElemOffset = (mInitialOffset + mObjWidth / 2 - ((mInitialOffset + mObjWidth / 2 - mFirstElemOffset) % mObjWidth));
            }
        }
    ...
    void moveElements(int steps) {
        if (steps < 0) {
            ...
            for (int i = getChildCount() + steps; i > 0 && i < getChildCount(); i++) {
                DateSlider.TimeObject newTo = mLabeler.add(((TimeView) getChildAt(i - 1)).getEndTime(), 1);
                ((TimeView) getChildAt(i)).setVals(newTo);
            }
            if (getChildCount() + steps <= 0) {
                for (int i = 0; i < getChildCount(); i++) {
                    DateSlider.TimeObject newTo = mLabeler.add(((TimeView) getChildAt(i)).getEndTime(), -steps);
                    ((TimeView) getChildAt(i)).setVals(newTo);
                }
            }
        } else if (steps > 0) {
